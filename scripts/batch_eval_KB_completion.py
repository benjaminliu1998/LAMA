...
def lowercase_samples(samples, use_negated_probes=False):
    ...
    for sample in samples:
        ...
        except KeyError: # for TREx, the json structure doesn't have "masked_sentences"
            for sentence in sample['evidences'][0]["masked_sentence"]:
                sentence = sentence.lower()
                sentence = sentence.replace(base.MASK.lower(), base.MASK)
                lower_masked_sentences.append(sentence)
            sample['evidences'][0]["masked_sentence"] = lower_masked_sentences
